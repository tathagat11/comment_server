...
function bindGroup(distName, Grp){
    ...
    for(let i = 0; i < Grp.length; i++){
        //if(i == 0)
                SELECTLIST += `<div style="padding: 3px 5px;"><input type="checkbox" id="dse_${i}" name="dseLists" value="${Grp[i].DSE}" checked onclick="checkchangeDSE(this)"/>&nbsp;&nbsp; ${Grp[i].DSE}</div>`;
        //else
            //SELECTLIST += `<div style="padding: 3px 5px;"><input type="checkbox" id="dse_${i}" name="dseLists" vlue="${Grp[i].DSE}"/>${Grp[i].DSE}</div>`;
    }
    SELECTLIST += '';
    $("#GroupSelect").html(`<div class="GRPSHEAD">Distributor</div><div style="padding: 3px 5px;"><input type="checkbox" checked onclick="selectedDSE(this)"/>&nbsp;&nbsp; ${distName}</div><hr/>`);
    $("#distSelect").html(SELECTLIST)
    $("#loader").hide();
}
function groupAndPlot(subGrp, Grp){
    if(Grp){
        ...
        for(let i = 0; i < Grp.length; i++){
            if(i == 0)
                    SELECTLIST += '<div class="mySelect"><input onchange="changeDistTrue()" name="dropcheckItems" type="checkbox" value="'+Grp[i].id+'" checked> '+Grp[i].Distributor+'</div>';
            else
...
async function getAndPlotPoints(replot){
    var Data = await _cacheManager.get(cacheInstance,"routeData");
    PlotPoints(Data.LocData, replot);
}
function PlotPoints(Data, replot) {
    ...
    for (var i = 0; i < Data.length; i++) {
        ...
        try{
            //var temp = Data[i].Id.split(":");
            var mainClust = `${Data[i].DSE}:${Data[i].Route}`; //`${Data[i].Route}`;
            var fltrClust = `${Data[i].DSE}`; //`${Data[i].Route}`;
            if(Checked_Clusters.indexOf(fltrClust) !== -1)
                continue;
            var colorInx = 0;
            var Type = 0;
            let lat = Data[i].loc.coordinates[1];
            let lng = Data[i].loc.coordinates[0];
            if(replot){
                if(Clusters.indexOf(fltrClust) == -1){
                    Clusters.push(fltrClust);
                }
                if(!clusterVAL.hasOwnProperty(mainClust)){
                    clusterCount++;
                    clusterVAL[mainClust] = {
                ...
                clusterVAL[mainClust].custCount++;

                clusterVAL[mainClust].locs.push({
                    cust:Data[i].CustCode,
                    loc:[lat,lng]
                });
            }

            var tempInx = parseInt(clusterVAL[mainClust].cluster, 10);
            colorInx = tempInx % 25;
            ...
            let marker = new L.marker([parseFloat(lat), parseFloat(lng)], {
                icon: myIcon
            })//.addTo(mymap);


            marker.bindPopup(`${Data[i].CustCode}:${Data[i].CustName}:${Data[i].DSE}:${Data[i].Route}`);
    ...
    showSummary(clusterVAL,replot);

}
async function showSummary(routeDat, replot){
    var summaryGrid = `
    <b>Route Summary</b>
    <hr/>
    <div style="overflow: auto;">
    <div id="routeSummaryGrid" class="summaryGrid">
    </div>
    </div>`
    $('#routeSummary').html(summaryGrid);
    for(var route in routeDat){
        var fltrClust = route.split(':')[0];
        if(Checked_Clusters.indexOf(fltrClust) !== -1)
            continue;
        var distance = 0;
        if(replot){
            try{
                var routeDist = await getfromOSRM(routeDat[route].locs);
                var distFromtrips = routeDist.trips[0].distance;
                var distToInt = parseInt(distFromtrips);
                distance = (distToInt/1000).toFixed(2);
                routeDat[route].distance = distance;
            }catch{distance=0;}
        }
        var tempInx = parseInt(routeDat[route].cluster, 10);
        var colorInx = tempInx % 25;
        var routesummary = `
        <div class="summaryGridItem">
            <table>
                <tr><th><span style="background: #${ColourValues[colorInx]};">&nbsp;&nbsp;&nbsp;&nbsp;</span> ${route}</th></tr>
                <tr><td>${routeDat[route].custCount} - outlets | ${routeDat[route].distance} - KM</td></tr>
            </table>
        </div>`
        $('#routeSummaryGrid').append(routesummary);
    }
